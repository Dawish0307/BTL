...
// Đặt file này trong: gialai_tourism/api/maps.php
// ==================================================
session_start();
...
// Lấy địa điểm có toạ độ (lat, lng không null)
// Bây giờ database đã có cột lat/lng nên không cần ánh xạ thủ công nữa
$result = $conn->query("
    SELECT id, name, slug, region, location, description, image_url, rating, lat, lng
    FROM destinations
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nav-logo {
      ...
      font-weight: 700; letter-spacing: 3px; color: var(--cream);
    ...
    .nav-links a {
      ...
      text-transform: uppercase; color: rgba(245,240,232,0.65); text-decoration: none; transition: color .2s;
    }
    .nav-links a:hover, .nav-links a.active { color: var(--cream); }
    .nav-links a.active { border-bottom: 1.5px solid var(--teal); padding-bottom: 2px; }
    .nav-auth { display: flex; gap: 10px; }
    .btn-login { padding: 7px 18px; border: 1.5px solid rgba(245,240,232,0.3); border-radius: 30px; font-size: 13px; color: var(--cream); text-decoration: none; }
    .btn-register { padding: 7px 18px; background: var(--teal); border-radius: 30px; font-size: 13px; font-weight: 600; color: var(--deep); text-decoration: none; }
    .btn-user { display: flex; align-items: center; gap: 8px; background: rgba(78,205,196,0.12); border: 1.5px solid rgba(78,205,196,0.3); border-radius: 30px; padding: 6px 14px; color: var(--cream); font-size: 13px; }
    .avatar { width: 24px; height: 24px; border-radius: 50%; background: var(--teal); color: var(--deep); font-size: 11px; font-weight: 700; display: flex; align-items: center; justify-content: center; }

    /* ── LAYOUT ──
       Toàn bộ trang = navbar (64px) + toolbar + bản đồ
       Dùng flexbox để bản đồ tự động chiếm phần còn lại
    */
    .main { padding-top: 64px; display: flex; flex-direction: column; height: 100vh; }

    /* ── TOOLBAR (thanh tìm kiếm + lọc) ── */
    .toolbar {
      flex-shrink: 0; /* không bị co lại */
      background: var(--deep2); border-bottom: 1px solid var(--border);
      padding: 12px 40px;
    ...
    .search-input {
      ...
      border-radius: 50px; color: var(--cream); font-size: 13px;
    ...
    .tag {
      ...
      color: rgba(245,240,232,0.5); font-size: 12px;
    ...
    /* ── BẢN ĐỒ ──
       ...
       Leaflet yêu cầu #map phải có width và height rõ ràng
    */
    .map-wrap { flex: 1; position: relative; }
    #map { width: 100%; height: 100%; }

    /* Loading overlay */
    .loading { position: absolute; inset: 0; background: var(--deep2); display: flex; flex-direction: column; align-items: center; justify-content: center; z-index: 400; transition: opacity .5s; }
    .loading.hide { opacity: 0; pointer-events: none; }
    .spinner { width: 32px; height: 32px; border: 2px solid rgba(78,205,196,0.2); border-top-color: var(--teal); border-radius: 50%; animation: spin .8s linear infinite; margin-bottom: 12px; }
    @keyframes spin { to { transform: rotate(360deg); } }

    /* Nút chuyển kiểu bản đồ */
    .map-btns { position: absolute; top: 12px; right: 12px; z-index: 500; display: flex; flex-direction: column; gap: 6px; }
    .map-btn { background: rgba(10,22,40,0.92); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; padding: 8px 12px; color: var(--cream); font-size: 12px; cursor: pointer; font-family: 'Inter', sans-serif; transition: all .2s; display: flex; align-items: center; gap: 5px; }
    ...
    .leaflet-popup-content-wrapper { background: #0f1f38 !important; border: 1px solid rgba(78,205,196,0.25) !important; border-radius: 12px !important; color: var(--cream) !important; font-family: 'Inter', sans-serif !important; box-shadow: 0 8px 32px rgba(0,0,0,0.5) !important; }
...
<nav class="navbar">
  <a href="../index.php" class="nav-logo">
    ⛰ GIA LAI <span>Đại Ngàn · Biển Xanh</span>
  </a>
  <ul class="nav-links">
...
<div class="main">
  ...
  <div class="toolbar">
    ...
    <div class="tags">
      <!-- data-r phải khớp giá trị ENUM trong cột region của database -->
...
<script>
// ── 1. DỮ LIỆU TỪ PHP ────────────────────────────────
// Database đã có lat/lng nên PHP trả về trực tiếp, không cần ánh xạ thủ công
const DATA = <?= $json ?>;
// Mỗi phần tử trong DATA có dạng:
// { id, name, slug, region, location, description, image_url, rating, lat, lng }

// ── 2. KHỞI TẠO BẢN ĐỒ ──────────────────────────────
const CENTER = [13.77, 109.22]; // Trung tâm (Quy Nhơn)
const ZOOM   = 10;

// Tắt nút zoom mặc định (ta sẽ thêm lại vào góc phải dưới)
const map = L.map('map', { zoomControl: false }).setView(CENTER, ZOOM);
L.control.zoom({ position: 'bottomright' }).addTo(map);

// ── 3. CÁC KIỂU NỀN BẢN ĐỒ ──────────────────────────
const TILES = {
...
// Chuyển kiểu bản đồ khi bấm nút
function setTile(name) {
  if (name === curTile) return;
  map.removeLayer(TILES[curTile]);
  TILES[name].addTo(map);
  curTile = name;
  const btnId = { street:'b1', satellite:'b2', topo:'b3' };
  document.querySelectorAll('.map-btn').forEach(b => b.classList.remove('active'));
  document.getElementById(btnId[name])?.classList.add('active');
}

// ── 4. TẠO MARKER VÀ POPUP ───────────────────────────

// Màu và biểu tượng theo loại địa điểm (khớp với ENUM trong DB)
const STYLE = {
  bien:   { color: '#0a3d62', emoji: '🌊' }, // xanh biển
  rung:   { color: '#1a4a2a', emoji: '🌿' }, // xanh lá
  lichsu: { color: '#4a2a1a', emoji: '🏛' }, // nâu lịch sử
};

// Tạo icon hình giọt nước (pin) bằng SVG
function makeIcon(region) {
  const s = STYLE[region] || { color: '#333', emoji: '📍' };
  const html = `<svg xmlns="http://www.w3.org/2000/svg" width="36" height="44" viewBox="0 0 36 44">
  ...
  return L.divIcon({ className:'', html, iconSize:[36,44], iconAnchor:[18,44], popupAnchor:[0,-44] });
}

// Tạo nội dung popup (hộp thông tin khi click marker)
function makePopup(d) {
  const stars = '★'.repeat(Math.floor(d.rating)) + '☆'.repeat(5 - Math.floor(d.rating));
  return `
    ${d.image_url ? `<img class="p-img" src="${d.image_url}" onerror="this.style.display='none'" alt="${d.name}">` : ''}
    <div class="p-name">${d.name}</div>
    <div class="p-loc">📍 ${d.location || ''}</div>
    <div class="p-desc">${(d.description || '').slice(0, 130)}…</div>
    <div class="p-star">${stars} <strong>${d.rating}</strong></div>
  `;
}

// ── 5. VẼ MARKERS LÊN BẢN ĐỒ ────────────────────────
let markers = []; // Lưu marker đang hiện để xóa khi lọc
...
function showMarkers(list) {
  clearMarkers();
  list.forEach(d => {
    // lat và lng lấy thẳng từ database (không cần ánh xạ nữa)
    if (!d.lat || !d.lng) return;

    const m = L.marker([parseFloat(d.lat), parseFloat(d.lng)], { icon: makeIcon(d.region) });
    m.bindPopup(makePopup(d), { maxWidth: 260, minWidth: 220 });
    m.addTo(map);
    markers.push(m);
  });
...
// ── 6. LỌC DỮ LIỆU ───────────────────────────────────
let filterRegion = 'all'; // Loại đang lọc
let filterText   = '';    // Từ khoá tìm kiếm
...
document.querySelectorAll('.tag').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    filterRegion = btn.dataset.r; // Lấy giá trị data-r="bien" / "rung" / "lichsu" / "all"
    applyFilter();
  });
});

// Ô tìm kiếm — debounce 300ms để không gọi filter liên tục khi gõ
let timer;
...
// ── 8. KHỞI CHẠY ─────────────────────────────────────
applyFilter(); // Vẽ tất cả ngay khi trang mở
